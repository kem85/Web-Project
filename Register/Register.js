document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateStrengthMeter() {
    ...
    strengthLabel.textContent = value.length
      ? labels[score]
      : "Password strength";
  ...
  confirmEmail.addEventListener("blur", () => {
    ...
    else if (value !== emailInput.value.trim())
      setError(confirmEmail, "Emails do not match.");
    else if (!isValidEmail(value))
      setError(confirmEmail, "Please enter a valid email address.");
  ...
  registerForm.addEventListener("submit", (event) => {
    ...
    if (!heightValue) {
      ...
      isNaN(heightValue) ||
      Number(heightValue) < 50 ||
      Number(heightValue) > 300
    ) {
